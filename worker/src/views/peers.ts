...
function peersPhone(peers: Peer[], byKey: Map<string, AgentPeer>, running: boolean, latency: Record<string, number[]>, talkers: Talker[] = []): Html {
  ...
  ${peers.map((p) => {
    ...
    return html`<div class="panel sheet m-only" id="sh-peer-${p.id}">
      ${sheetHead(p.name)}
      <p>${statusCell(p, live, running, online)}${needsConfigPill(p)}${p.routes ? html` <span class="pill up">site: ${p.routes}</span>` : ""}${p.home_lan && !p.full_tunnel ? html` <span class="pill idle">+ home</span>` : ""}${p.full_tunnel ? html` <span class="pill idle">full tunnel</span>` : ""}${p.azure_vnet && !p.full_tunnel ? html` <span class="pill idle">+ azure</span>` : ""}${p.tunnel_dns || p.full_tunnel ? html` <span class="pill idle">tunnel DNS</span>` : ""}${lifeMarkers(p)}</p>
      <dl class="m-kv">
  ...
  ${peers.length
    ? html`<table class="rows stack">
      ...
      ${peers.map((p) => {
        ...
        const online = !!live && peerOnline(live);
        return html`<tr>
          <td class="lead"><b>${p.name}</b>${needsConfigPill(p)}${p.routes ? html` <span class="pill up">site: ${p.routes}</span>` : ""}${p.full_tunnel ? html` <span class="pill idle">full tunnel</span>` : ""}${p.azure_vnet && !p.full_tunnel ? html` <span class="pill idle">+ azure</span>` : ""}${p.home_lan && !p.full_tunnel ? html` <span class="pill idle">+ home</span>` : ""}${p.tunnel_dns || p.full_tunnel ? html` <span class="pill idle">tunnel DNS</span>` : ""}${lifeMarkers(p)}<div class="key" title="${p.public_key}">${p.public_key}</div></td>
