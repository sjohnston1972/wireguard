    ...
    ? html`<table class="rows">
      ...
      <tbody>${o.runs.map((r) => {
        ...
        return html`<tr>
          ...
          <td><b>${r.action === "apply" ? "Deploy" : "Tear down"}</b>${r.public_ip ? html`<div class="key">${r.public_ip}</div>` : ""}</td>
          <td><span class="pill ${kind}">${r.status}</span>${r.github_run_url ? html` <a class="small" href="${r.github_run_url}" target="_blank" rel="noopener">log</a>` : ""}</td>
          <td>${duration(r.started_at ?? r.requested_at, r.finished_at)}</td>
          <td class="num">${cost === null ? html`<span class="faint">—</span>` : html`~${gbp(cost)}`}</td>
          <td class="small">${r.requested_by ?? ""}</td>
          <td class="small muted">${r.reason ?? ""}${r.error ? html`<div style="color:var(--down)">${r.error}</div>` : ""}</td>
        </tr>`;
      })}</tbody></table>`
...
<section>
  ...
  <div class="table-wrap">
  ${o.alerts.length
    ? html`<table class="rows"><thead><tr><th>When</th><th>Kind</th><th>What happened</th></tr></thead><tbody>
      ${o.alerts.map((a) => html`<tr><td>${fmtTime(a.at)}</td><td><span class="pill ${a.kind === "failure" || a.kind === "drift" || a.kind === "cost_guard" || a.kind === "unreachable" ? "down" : a.kind === "deploy" || a.kind === "destroy" ? "up" : "idle"}">${a.kind.replace("_", " ")}</span></td><td>${a.message}</td></tr>`)}
      </tbody></table>`
