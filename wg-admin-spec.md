...
| Dashboard data backups | Once a day the watchman writes clients, firewall rules, published ports, profiles, schedules, alert phones and Settings changes to R2 `config-backups/<date>.json`, keeping the last 30. Settings > Backups shows both kinds, downloads an export, and restores one (preview, then type "restore"; refused while a run is in progress) | Losing D1 does not mean reissuing every client config |
| Worker storage | D1 (SQLite) for runs, peers, alerts and settings; one Durable Object for the run lock AND the live status snapshot (atomic merges; KV proved eventually consistent and lost a field under concurrent writes); KV only for a read mirror, the Azure token cache and small flags | All free tier |
