...
| Who can reach wg-admin | Cloudflare Access app "wg-admin", allowing only stevie.johnston@gmail.com via one-time PIN | MFA-grade login without writing an auth system. App already exists |
...
- [ ] GITHUB_TOKEN: still REPLACE_ME. **Now the only thing between the dashboard and a working Deploy button.** After minting: put it in .env, run `npm run secrets -- --worker`. github.com > Settings > Developer settings > Fine-grained tokens, repository = sjohnston1972/wireguard only, permissions Actions: Read and write, Contents: Read
